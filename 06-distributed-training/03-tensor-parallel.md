...
数据并行的前提是每张卡能装下完整的模型。但当模型超过某个规模时，单层的权重矩阵就可能超过单卡显存。比如一个 hidden_size=16384 的模型，它的 FFN 层有一个 16384×43000 的权重矩阵，单精度就要 2.7 GB——加上 QKV 投影、输出层、梯度和优化器状态，单卡完全放不下。

张量并行的思路是：**把这个大矩阵切成几块，分别放在不同的卡上计算，然后通过 AllReduce 汇合结果**。类似于 4 个人合作做一道大题——每人算一部分，最后合并。但这要求每次合并都很快（因为每层都要合并一次），所以 TP 必须放在 NVLink 互联的机内卡上。
...
TP 常常优先放在机内，不是因为“跨机绝对不行”，而是因为它几乎每层都要通信，且通信在关键路径上。
单机内 NVLink/NVSwitch 的有效带宽和时延通常明显优于跨机网络；跨机 TP 只有在网络足够强、切分粒度合适、并且通信能够被部分隐藏时才比较可行。
```
