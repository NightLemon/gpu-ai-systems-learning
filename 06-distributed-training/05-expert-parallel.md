...
> Mixture of Experts（混合专家）让模型参数量暴增但计算量可控——专家并行解决其分布式挑战。

## MoE 的核心思想

到目前为止我们讲的模型都是“Dense”的——每个 token 都要过模型的所有参数。但如果一个模型有 8 个 FFN 层的“专家”，每个 token 只选择其中 2 个使用呢？总参数量是 8 倍，但每个 token 的计算量只有 2/8 = 25%。这就是 MoE 的核心价值：**用更少的计算获得更大模型的能力**。

Mixtral 8x7B 就是这个思路：总参数 47B，但每个 token 只激活 12.9B，效果接近 LLaMA 2 70B。

但 MoE 给系统带来的挑战是：如何把 8 个 expert 分布到不同 GPU 上，并且高效地把 token 发送到对应的 expert 所在的 GPU？这就是专家并行要解决的问题。它的通信模式是 All-to-All（每张卡向每张其他卡发送不同的数据），比 AllReduce 更难优化。
...
MoE 不是通用最优解。它更适合下面这类场景：

- 训练预算非常紧，希望用更低 FLOPs 换更大参数规模
- 团队能接受更高的训练复杂度和系统复杂度
- 推理场景允许做专门优化，或者推理量不是主要成本中心
- 有足够能力处理 router 稳定性、load balance 和 all-to-all 问题

相反，如果你的主要压力在推理成本、上线稳定性或服务简单性，dense 模型往往更现实。
...
1. 你是在优化**训练 FLOPs**，还是在优化**线上服务成本**？
2. 你的网络拓扑能不能承受高频 all-to-all？
3. 你的 batch 是否足够大，能让 expert GEMM 不至于过小？
4. 你是否能接受更复杂的部署、监控和排障链路？

如果前两个问题答不好，MoE 往往不是当前阶段最好的答案。
...
A: 如果你是中小规模团队、网络条件一般、线上推理是主成本、或者模型版本切换很频繁，通常先把 dense 路线做到位更合理。MoE 的收益常常建立在更强的系统工程能力之上。
