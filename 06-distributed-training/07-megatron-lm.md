...
前面几节你已经学了 DP、TP、PP 三种并行策略。但真正训练一个 175B 的模型时，你需要同时使用这三种策略（以及序列并行、分布式优化器等辅助技术），并且要把它们**映射到实际的 GPU 集群拓扑上**（哪些卡做 TP、哪些卡做 PP、哪些卡做 DP）。

Megatron-LM 是 NVIDIA 实现这套“3D 并行”的框架。GPT-3、LLaMA 等标志性大模型的训练都参考了 Megatron 的并行策略。学习 Megatron-LM 不只是学一个框架的用法，更是理解“如何将并行策略落地到真实硬件上”的标杆案例。
