...
你训练一个 7B 模型，发现单卡 80GB 放不下（模型 + 梯度 + 优化器状态 = 112 GB）。最自然的想法是“加卡”——但加卡有不同的加法。

**数据并行（DP）**是最简单的方式：每张卡拿一份完整的模型副本，各自用不同的数据算梯度，然后 AllReduce 同步梯度。但这意味着每张卡都要存一份 112 GB 的完整状态——显存浪费严重。

**ZeRO 的核心洞察**：4 张卡上的优化器状态是一模一样的，为什么每张卡都存一份？把它切成 4 份，每张卡只存 1/4，显存立刻省下来。这就是 ZeRO Stage 1。继续切梯度（Stage 2）、切参数（Stage 3），显存节省越来越多，但通信量也相应增加。

这个“显存 vs 通信”的 trade-off 是理解所有数据并行策略的主线。
