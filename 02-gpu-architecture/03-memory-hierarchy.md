...
GPU 的计算单元（Tensor Core）每秒能做数百到上千 TFLOPS 级别的矩阵运算，但 HBM 显存每秒只能搬运几 TB 数据。以 H100 为例，BF16 dense 口径约 989 TFLOPS、HBM3 带宽约 3.35 TB/s，对应 roofline 临界点约 **295 FLOPs/Byte**；如果使用带结构化稀疏的宣传口径，临界点约 **590 FLOPs/Byte**。绝大多数操作达不到这个比值——也就是说，**GPU 的计算单元大部分时间在等数据**。
