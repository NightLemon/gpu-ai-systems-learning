...
很多初学者写的第一个 CUDA 程序，跑完后发现 GPU 版比 CPU 版还慢。为什么？因为大部分时间花在了数据从 CPU 内存搬运到 GPU 显存的过程中（通过 PCIe 总线，带宽只有 ~32 GB/s），而不是在 GPU 计算上。

这就像你招了一支顾问团队帮你干活，但每次都要开车 2 小时把材料送到他们办公室，再开 2 小时把结果拿回来——实际干活只要 10 分钟，但路上花了 4 小时。

所以 CUDA 内存管理的核心原则是：**尽量减少 CPU↔GPU 之间的数据搬运，尽量让数据留在 GPU 上处理完再传回**。当必须搬运时，用 Pinned Memory + 异步传输 + Stream 重叠 来最小化影响。
## 核心概念
