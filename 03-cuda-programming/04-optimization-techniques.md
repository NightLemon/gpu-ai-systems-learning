...
很多人优化 CUDA 的思路是：“我觉得这里慢，加个 Shared Memory 试试”。这样做的问题是——你在猜。

GPU 上一个 kernel 可能慢的原因有很多：显存访问模式不对、Occupancy 太低、分支发散、数据传输阻塞……每种原因的优化方向完全不同。如果不先确定瓶颈在哪，很可能花大力气优化了一个不是瓶颈的地方。

所以专业的做法是：**先用 Profiler 拍一张“照片”（采集性能数据），看清楚瓶颈在哪，再对症下药**。这就是“Profile-guided Optimization”的思维。
